    ...
    public class ExternalController : Controller
    {
        ...
        public async Task<IActionResult> Callback()
        {
            ...
            if (user == null)
            {
                ...
                user = await AutoProvisionUser(provider, providerUserId, result);
            }
            else
            {
        ...
        private async Task<(IdentityUser user,string provider, string providerUserId, IEnumerable<Claim> claims)> FindUserFromExternalProvider(AuthenticateResult result)
        {
            ...
            var userIdClaim = externalUser.FindFirst(JwtClaimTypes.Subject) ??
                              ...
                              throw new Exception("Unknown userid can not proceed further");

            var userEmailClaim = externalUser.FindFirst(ClaimTypes.Email) ??
                              throw new Exception("Unknown Email Can not proceed further");
        ...
        {
            // code for the external user login registration stratagy
            // for use cases follow the link ==> https://chsakell.com/2019/07/28/asp-net-core-identity-series-external-provider-authentication-registration-strategy/
            ...
            var userEmailClaim = result.Principal.FindFirst(ClaimTypes.Email) ??
                              throw new Exception("Unknown Email Can not proceed further");

            var user = new IdentityUser(userEmailClaim.Value)
            {
                NormalizedEmail = userEmailClaim.Value,
                NormalizedUserName = userEmailClaim.Value,
            ...
            if (userResult.Succeeded && newLoginResult.Succeeded)
            {
                return user;
            }
            else
            {
                throw new Exception("Can not create user from external provider");
            }
        }
